...
function Add() {
  ...
  else { alert("Ener atleast IMEI, Model and Count before trying to ADD") }
...
function Edit() {
  ...
  else { alert("Enter IMEI number before trying to Edit") }
...
function Delete() {
  ...
  else { alert("Ener IMEI number before trying to Delete") }
}
...
function goToBilling() {
  if (document.getElementById("IMEI").value || (document.getElementById('IsSpares').checked && document.getElementById("Model").value)) {
    ...
    else {//alert("Search Inside Billing Table:"+JSON.parse(xmlHttp1.responseText).length)
